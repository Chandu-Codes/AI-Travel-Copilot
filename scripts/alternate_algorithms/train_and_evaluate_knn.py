...
print("==================================================================")
print("🔍 TRAINING & EVALUATING K-NEAREST NEIGHBORS (KNN) FOR FLIGHT PRICES")
print("==================================================================\n")
...
# Identical sample & seed as RF and Linear Regression for a fair comparison
if len(df_f) > 40000:
...
# Preprocessor: Scaling is mandatory for distance-based algorithms like KNN
...
# KNN Regressor with distance weighting and multi-core processing
...
print("Training (Indexing) KNN Regressor (k=5, weights='distance')...")
...
# Evaluate on Test Set (Measuring inference time)
print("Evaluating on 8,000 test samples (computing all distance matrices)...")
...
# Save KNN Model
knn_model_path = "models/alternate_benchmarks/flight_price_knn_model.joblib"
joblib.dump(knn_pipeline, knn_model_path)
print(f"✓ KNN model saved to: {knn_model_path}")

# Load RF and Linear Regression baseline metrics
...
# Side-by-side test case predictions
rf_model = joblib.load("models/flight_price_model.joblib") if os.path.exists("models/flight_price_model.joblib") else None
lin_model = joblib.load("models/flight_price_linear_model.joblib") if os.path.exists("models/flight_price_linear_model.joblib") else None

case_results = []
for case in sample_cases:
    knn_val = float(knn_pipeline.predict(case["input"])[0])
    ...
    case_results.append({
        ...
        "knn_prediction": round(knn_val, 2),
...
comparison_report = {
    ...
    "knn_metrics": {
        "model_type": "KNeighborsRegressor (k=5, weights=distance, algorithm=auto)",
        ...
        "file_path": knn_model_path
    },
    "comparison_vs_other_models": {
        "random_forest": {
            "r2_score": rf_r2,
            "mae_inr": rf_mae,
            "rmse_inr": rf_rmse,
            "inference_speed_comparison": f"RF is {round(pred_time / 0.18, 1)}x faster during inference than KNN"
        },
        "linear_regression": {
            "r2_score": lin_r2,
            "mae_inr": lin_mae,
...
report_path = "models/alternate_benchmarks/knn_evaluation.json"
with open(report_path, "w", encoding="utf-8") as f:
    json.dump(comparison_report, f, indent=2)

print("\n==================================================================")
print("📊 KNN EVALUATION RESULTS SUMMARY")
print("==================================================================")
print(f"KNN R² Score              : {r2:.4f}")
print(f"KNN MAE                   : ₹{mae:,.2f}")
print(f"KNN RMSE                  : ₹{rmse:,.2f}")
print(f"KNN MAPE                  : {mape:.2f}%")
print(f"KNN Test Inference Time   : {pred_time:.2f} seconds for 8,000 queries")
print("------------------------------------------------------------------")
